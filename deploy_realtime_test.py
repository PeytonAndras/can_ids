...
class CandumpStreamReader:
    """Reads CAN frames from candump-style text output (e.g., from console logs)"""
    
    def __init__(self, file_handle) -> None:
        self.file_handle = file_handle
    
    def frames(self) -> Iterable[Frame]:
        """Parse candump format: can0  01A   [8]  0F A3 00 00 12 34 56 78"""
        for line in self.file_handle:
            ...
            try:
                # Parse: "can0  01A   [8]  0F A3 00 00 12 34 56 78"
                parts = line.split()
                if len(parts) < 4:
                    continue
                
                # Extract CAN ID (handle with/without leading zeros)
                can_id_str = parts[1]
                arbitration_id = int(can_id_str, 16)
                
                # Extract DLC from [8] format
                dlc_str = parts[2].strip("[]")
                dlc = int(dlc_str)
                ...
                logging.debug("Skipping malformed candump line: %s (error: %s)", line, exc)
                continue
...
def run_pipeline(config: DeploymentConfig, live_channel: Optional[str], bustype: str, replay_path: Optional[Path], candump_stream: bool) -> None:
    ...
    def frame_iter() -> Iterable[Frame]:
        if candump_stream:
            logging.info("Reading candump-style CAN frames from stdin")
            candump_reader = CandumpStreamReader(sys.stdin)
            return candump_reader.frames()
        ...
        raise ValueError("Either --replay, --can-channel, or --candump-stream must be provided")
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument("--candump-stream", action="store_true", help="Read candump-style CAN frames from stdin (pipe from Hybrid script)")
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    if args.can_channel is None and config.replay_csv is None and not args.candump_stream:
        raise ValueError("Either --can-channel, --replay, or --candump-stream must be specified")

    replay_path = config.replay_csv
    live_channel = args.can_channel
    run_pipeline(config, live_channel=live_channel, bustype=args.bustype, replay_path=replay_path, candump_stream=args.candump_stream)
